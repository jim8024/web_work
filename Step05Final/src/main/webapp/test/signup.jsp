...
<%
	...
	if(hobby!=null){
		...
		for(int i = 0; i<hobby.length; i++){
			...
			String tmp = hobby[i];
			hobbyResult=hobbyResult+tmp;//hobbyResult+=tmp;
			if(i != hobby.length-1){
				hobbyResult += "/";  //비디에 저장하고 스트링 배열로 만들어서 쓴다.
			}
		}
	}
	//문자열에서 특정 문자 "/" 로 구분해서 Stringp[] 얻어내기
	String[] result=hobbyResult.split("/");
	//문자열에 특정 문자가 포함되어 있는지 여부 알아내기
	boolean isContain=hobbyResult.contains("xxx");
	
	//null 이면 체크를 안한것이고  null이 아니면 체크를 한 것이다.
	String allowEmail=request.getParameter("allowEmail");
	String allowMessage=request.getParameter("allowMessage");
	/*
		textarea에 일역한 문자열은 개행,tab,공백기호가 모두 포함되어 있다
		해당 문자열을 DB에 저장했다가 textarea에 출력하고 싶으면
...
<body>
	<div class="container">
        ...
        <form action="signup.jsp" method="post">
            ...
            <div>
            	<label for="comment" class="form-label">하고싶은 말</label>
