...
<%
	...
	/*
		2.DB에서 다운로드 시켜줄 파일의 정보를 읽어온다.
			...
			업로드 당시 원본 파일 명은 무엇인지=>orgFileName
			파일의 사이즈는 어떻게 되는지=>fileSize
			다운로드 시켜주기 위해 3가지가 필요
	*/
    ...
    String path=application.getRealPath("/upload") + File.separator+saveFileName;//파일구분자(운영체제에 따라 바뀜)
    ...
    String encodedName=URLEncoder.encode(orgFileName, "utf-8");
	 //파일명에 공백이있는 경우 처리 
	 encodedName=encodedName.replaceAll("\\+"," "); //역스레쉬 하나이상 있는 것 공백처리
    ...
    BufferedOutputStream bos=
       new BufferedOutputStream(response.getOutputStream());//reponse,request는 기본 객체라서 .찍어서 바로 사용가능
